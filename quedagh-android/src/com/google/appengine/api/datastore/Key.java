 ...
 * 
 * A datastore GUID. A Key instance uniquely identifies an entity across all apps, and includes all
 * information necessary to fetch the entity from the datastore with
 * {@code DatastoreService.get(Key)}.
 * <p>
 * You can create {@code Key} objects directly by using {@link KeyFactory#createKey} or
 * {@link #getChild}.
 * <p>
 * You can also retrieve the {@code Key} automatically created when you create a new {@link Entity},
 * or serialize {@code Key} objects, or use {@link KeyFactory} to convert them to and from websafe
 * String values.
 * 
 * @see KeyFactory
 * 
...
public final class Key implements Serializable, Comparable<Key>
{
	...
	private transient String appIdNamespace;
	...
	Key(String kind, Key parentKey, long id, String appIdNamespace)
	{
	...
	Key(String kind, Key parentKey, String name, String appIdNamespace)
	{
		this(kind, parentKey, NOT_ASSIGNED, name, appIdNamespace);
	}

	Key(String kind, Key parentKey, long id, String name, String appIdNamespace)
	{
		...
		if (name != null)
		{
			...
			else if (id != NOT_ASSIGNED) { throw new IllegalArgumentException(
	...
	{
		if (value == null || parentKey == null) { return value; }
	...
	private static void validateAppIdNamespace(Key parentKey, String appIdNamespace)
	{
		if (parentKey != null && appIdNamespace != null && parentKey.getAppIdNamespace() != null
				&& !(parentKey.getAppIdNamespace().equals(appIdNamespace))) { throw new IllegalArgumentException(
	...
	private void appendToString(StringBuffer buffer)
	{
		...
		else
		{
			...
			{
				String namespace = appIdNamespace;
	 ...
	 *            if NOT_ASSIGNED should be considered as different from any other.
	...
	boolean equals(Object object, boolean considerNotAssigned)
	{
		if (object instanceof Key)
		{
			...
			if (!appIdNamespace.equals(key.appIdNamespace)) { return false; }
			if (considerNotAssigned && name == null && id == NOT_ASSIGNED && key.id == NOT_ASSIGNED) { return false; }
			if (id != key.id || !kind.equals(key.kind)) { return false; }
			if (parentKey != key.parentKey
					&& (parentKey == null || !parentKey.equals(key.parentKey, considerNotAssigned))) { return false; }
			return true;
		}
		return false;
	}
	...
	/**
	 * Returns the identifier of the application in which this {@code Key} is stored, or
	 * {@code null} if the current application should be used.
	 */
	String getAppIdNamespace()
	{
	 ...
	 * @param id
	 *            the numeric identifier of the key in {@code kind}, unique for this parent
	 */
	public Key getChild(String kind, long id)
	{
		if (!isComplete()) { throw new IllegalStateException("Cannot get a child of an incomplete key."); }
	 ...
	 *            the kind of the child key to create
	 * @param name
	 *            the name of the key in {@code kind}, as an arbitrary string unique for this parent
	 */
	...
	void setId(long id)
	{
		if (name != null) { throw new IllegalArgumentException("Cannot set id; key already has a name."); }
		this.id = id;
	}
	...
	/**
	 * Build an {@code Iterator} for the given {@code Key} that returns all the keys that compose
	 * the key, starting with the topmost ancestor and proceeding down to the most recent child.
	 */
	...
	/**
	 * Compares two {@code Key} objects. The algorithm proceeds as follows: Turn each {@code Key}
	 * into an iterator where the first element returned is the top-most ancestor, the next element
	 * is the child of the previous element, and so on. The last element will be the {@code Key} we
	 * started with. Once we have assembled these two iterators (one for 'this' and one for the
	 * {@code Key} we're comparing to), consume them in parallel, comparing the next element from
	 * each iterator. If at any point the comparison of these two elements yields a non-zero result,
	 * return that as the result of the overall comparison. If we exhaust the iterator built from
	 * 'this' before we exhaust the iterator built from the other {@code Key}, we return less than.
	 * An example:
	 * 
	 * app1.type1.4.app1.type2.9 < app1.type1.4.app1.type2.9.app1.type3.2
	 * 
	 * If we exhaust the iterator built from the other {@code Key} before we exhaust the iterator
	 * built from 'this', we return greater than. An example:
	 * 
	 * app1.type1.4.app1.type2.9.app1.type3.2 > app1.type1.4.app1.type2.9
	 * 
	 * The relationship between individual {@code Key Keys} is performed by comparing app followed
	 * by kind followed by id. If both keys are assigned names rather than ids, compares names
	 * instead of ids. If neither key has an id or a name we return an arbitrary but consistent
	...
	private static int compareToInternal(Key thisKey, Key otherKey)
	{
		...
		if (!thisKey.isComplete() && !otherKey.isComplete()) { return compareToWithIdentityHash(thisKey, otherKey); }

		if (thisKey.getId() != NOT_ASSIGNED)
		{
			if (otherKey.getId() == NOT_ASSIGNED) { return -1; }
